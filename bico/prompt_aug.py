...
CAP_EN_SYS_PROMPT = (
    """You are a video captioning specialist whose goal is to write high-quality English prompts by referring to the details of the user's input videos, making them complete and expressive according to the input videos. You need to adhere to the formatting of the examples provided strictly.\n"""
    """Task Requirements:\n"""
    """1. Avoid overly brief output. Provide details in the video, making the caption more complete and visually appealing;\n"""
    """2. Improve the characteristics of the main subject (such as appearance, expression, quantity, ethnicity, posture, etc.), rendering style, spatial relationships, and camera angles;\n"""
    ...
    """4. The caption should provide a precise and detailed style description. You need to carefully analyze the style of the user's provided video and use that as a reference for captioning;\n"""
    """6. You need to emphasize movement information in the input and different camera angles;\n"""
    """7. Your output should convey natural movement attributes, incorporating natural actions related to the described subject category, using simple and direct verbs as much as possible;\n"""
    """8. You should refer the detailed information in the video, such as character actions, clothing, backgrounds, and emphasize the details in the video;\n"""
    """9. Control the caption to around 80-100 words.\n"""
    """10. No matter what language the user inputs, you must always output in English.\n"""
...
CONCEPT_IMG_EN_SYS_PROMPT = (
    ...
    """{\n"""
    """\"spatial\": [\"brown cat\", \"sunglasses\", \"sketch\", \"sunny\", \"grassland\", \"human pose\"]\n"""
    """}\n"""
    """Please output in JSON format (pure text, without markdown formatting)."""
)

CONCEPT_EN_SYS_PROMPT = (
    ...
    """{\n"""
    """\"spatial\": [\"brown cat\", \"sunglasses\", \"sketch\", \"sunny\", \"grassland\", \"human pose\"]\n"""
    """\"temporal\": [\"jumping\", \"running\", \"falling\", \"gently flowing\", \"bright to dark\", \"near to far\"]\n"""
    """}\n"""
    """Please output in JSON format (pure text, without markdown formatting)."""
)

AUG_IMG_EN_SYS_PROMPT = (
    ...
    """Example of the English captions:\n"""
    """1. A boat in a river, with trees and houses on the riverbank, and a foggy sky.\n"""
    ...
    def video_caption(
        ...
        **kwargs,
        ...
        if video_path is not None:
            ...
            raise ValueError("Either video_path or images_dir should be provided.")
...
def generate_video_caption():
    # video_dirs = [
    #     "/data-tmp/xhkong/data/DAVIS-2017-trainval-480p",
    #     "/data-tmp/xhkong/data/DAVIS-2017-test-dev-480p",
    #     "/data-tmp/xhkong/data/DAVIS-2017-test-challenge-480p",
    #     "/data-tmp/xhkong/data/DAVIS-2019-Unsupervised-test-challenge-480p",
    #     "/data-tmp/xhkong/data/DAVIS-2019-Unsupervised-test-dev-480p",
    # ]
    # images_dirs = [
    #     os.path.join(video_dir, "JPEGImages/480p") for video_dir in video_dirs
    # ]
    video_dirs = [
        "/data-tmp/xhkong/data/Test",
    ]
    images_dirs = [
        os.path.join(video_dir, "JPEGImages_new") for video_dir in video_dirs
    ]
    ...
    for i, images_dir in enumerate(images_dirs):
        print(
            f"### Processing {i + 1}th video dir (Total {len(images_dirs)}): {images_dir} ..."
        )

        result_dict = {}
        image_seqs_names = os.listdir(images_dir)
        for image_seq_name in tqdm(image_seqs_names):
            images_path = os.path.join(images_dir, image_seq_name)
            qwen_result = qwen_prompt_expander.video_caption(
                CAP_EN_SYS_PROMPT, images_dir=images_path, seed=seed
            )
            result_dict[image_seq_name] = qwen_result.prompt

        with open(
            os.path.join(video_dirs[i], "video_caption.json"), "w", encoding="utf-8"
        ) as f:
...
def extract_concept_img():
    ...
    for img_dir in img_dirs:
        files = os.listdir(os.path.join(dataset_path, img_dir))
        for file in files:
            if file.lower().endswith((".jpg", ".jpeg", ".png", ".bmp")):
                img_list.append(
                    {"img_name": file, "dir": os.path.join(dataset_path, img_dir)}
                )
    ...
    for img_dict in tqdm(img_list):
        save_name = os.path.join(
            img_dict["dir"],
            f"{os.path.splitext(img_dict['img_name'])[0]}.json",
        )
        if os.path.exists(save_name):
            print(f"### Skip existing {save_name}")
            continue

        img_path = os.path.join(img_dict["dir"], img_dict["img_name"])
        try:
            qwen_result = qwen_prompt_expander.img_caption(
                CONCEPT_IMG_EN_SYS_PROMPT, image=img_path, seed=seed
            )
            json_str = qwen_result.prompt[
                qwen_result.prompt.find("{") : qwen_result.prompt.rfind("}") + 1
            ]
            ...
            print(f"### Failed to process {img_dict['img_name']}")
...
def extract_concept_video():
    ...
    for video_dir in video_dirs:
        files = os.listdir(os.path.join(dataset_path, video_dir))
        for file in files:
            if file.lower().endswith((".mp4", ".avi", ".mov", ".mkv")):
                video_list.append(
                    {"video_name": file, "dir": os.path.join(dataset_path, video_dir)}
                )
    ...
    for video_dict in tqdm(video_list):
        save_name = os.path.join(
            video_dict["dir"],
            f"{os.path.splitext(video_dict['video_name'])[0]}.json",
        )
        if os.path.exists(save_name):
            print(f"### Skip existing {save_name}")
            continue
        ...
        #     video_dict["dir"], os.path.splitext(video_dict["video_name"])[0]
        # )
        video_path = os.path.join(video_dict["dir"], video_dict["video_name"])
        try:
            qwen_result = qwen_prompt_expander.video_caption(
                CONCEPT_EN_SYS_PROMPT, video_path=video_path, seed=seed
            )
            json_str = qwen_result.prompt[
                qwen_result.prompt.find("{") : qwen_result.prompt.rfind("}") + 1
            ]
            ...
            print(f"### Failed to process {video_dict['video_name']}")
...
def generate_prompts_img():
    ...
    for img_dir in img_dirs:
        ...
        for file in files:
            if file.lower().endswith((".jpg", ".jpeg", ".png", ".bmp")):
                ...
                json_name = f"{main_file_name}.json"
                if os.path.exists(os.path.join(dataset_path, img_dir, json_name)):
                    img_list.append(
                        {
                            "img_name": file,
                            "dir": os.path.join(dataset_path, img_dir),
                            "json": json_name,
                        }
                    )
    ...
    for img_dict in tqdm(img_list):
        save_name = os.path.join(
            img_dict["dir"],
            f"{os.path.splitext(img_dict['img_name'])[0]}_prompts.csv",
        )
        if os.path.exists(save_name):
            print(f"### Skip existing {save_name}")
            continue

        json_path = os.path.join(img_dict["dir"], img_dict["json"])
        img_path = os.path.join(img_dict["dir"], img_dict["img_name"])
        try:
            ...
            for i in range(num_prompts):
                ...
                csv_data.append([img_dict["img_name"], qwen_result.prompt])
...
def generate_prompts_video_per_frame():
    ...
    for video_dir in video_dirs:
        ...
        for file in files:
            if file.lower().endswith((".mp4", ".avi", ".mov", ".mkv")):
                main_file_name = os.path.splitext(file)[0]
                json_name = f"{main_file_name}.json"
                if os.path.exists(os.path.join(dataset_path, video_dir, json_name)):
                    video_list.append(
                        {
                            "video_name": file,
                            "dir": os.path.join(dataset_path, video_dir),
                            "frames_dir": os.path.join(
                                dataset_path, video_dir, main_file_name
                            ),
                            "json": json_name,
                        }
                    )
    ...
    for video_dict in tqdm(video_list):
        save_name = os.path.join(
            video_dict["dir"],
            f"{os.path.splitext(video_dict['video_name'])[0]}_spatial_prompts.csv",
        )
        if os.path.exists(save_name):
            print(f"### Skip existing {save_name}")
            continue

        json_path = os.path.join(video_dict["dir"], video_dict["json"])
        img_list = os.listdir(video_dict["frames_dir"])
        img_list = sorted(
            img_list,
            key=lambda x: int(os.path.splitext(os.path.basename(x))[0]),
        )[:max_num_frames]
        try:
            with open(json_path, "r", encoding="utf-8") as f:
                json_dict = json.load(f)
                json_dict.pop("temporal", None)
                json_str = json.dumps(json_dict, ensure_ascii=False)

            csv_data = [["video", "prompt"]]
            for i, img_name in enumerate(tqdm(img_list)):
                img_path = os.path.join(video_dict["frames_dir"], img_name)
                qwen_result = qwen_prompt_expander.img_caption(
                    AUG_IMG_EN_SYS_PROMPT,
                    image=img_path,
                    seed=seed + i,
                ...
                video_name = os.path.splitext(video_dict["video_name"])[0]
                csv_data.append([os.path.join(video_name, img_name), qwen_result.prompt])
...
def generate_prompts_video():
    ...
    for video_dict in tqdm(video_list):
        save_name = os.path.join(
            video_dict["dir"],
            f"{os.path.splitext(video_dict['video_name'])[0]}_prompts.csv",
        )
        if os.path.exists(save_name):
            print(f"### Skip existing {save_name}")
            continue
        ...
        try:
            ...
            for i in range(num_prompts):
                ...
                csv_data.append([video_dict["video_name"], qwen_result.prompt])
...
if __name__ == "__main__":
    ...
    generate_prompts_img()

    # extract_concept_video()

    # generate_prompts_video_per_frame()
