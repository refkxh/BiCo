...
from ..models.wan_video_dit import WanModel, RMSNorm, sinusoidal_embedding_1d, ConceptAdapter
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.in_iteration_models = ("dit", "motion_controller", "vace", "animate_adapter", "concept_adapter_dict")
        self.in_iteration_models_2 = ("dit2", "motion_controller", "vace2", "animate_adapter", "concept_adapter_dict")
        ...
        self.concept_adapter_dict: Optional[nn.ModuleDict] = None
    ...
    def training_loss(self, **inputs):
        if inputs["concept_adapter_stage"] != "none":
            timestep_seg_ratio = 0.875  # TODO: make it a hyperparameter
            left_boundary = 0
            right_boundary = self.scheduler.num_train_timesteps
            mid_boundary = int(self.scheduler.num_train_timesteps * timestep_seg_ratio)

            major_seg_prob = 0.8  # TODO: make it a hyperparameter
            major_seg = torch.rand(1).item() < major_seg_prob

            if inputs["concept_adapter_stage"] == "global":
                if major_seg:
                    max_timestep_boundary = right_boundary
                    min_timestep_boundary = mid_boundary
                else:
                    max_timestep_boundary = mid_boundary
                    min_timestep_boundary = left_boundary
            elif inputs["concept_adapter_stage"] == "all":
                if major_seg:
                    max_timestep_boundary = mid_boundary
                    min_timestep_boundary = left_boundary
                else:
        ...
        global_only = inputs["concept_adapter_stage"] == "global"
        noise_pred = self.model_fn(
            **inputs,
            timestep=timestep,
            concept_adapter_global_only=global_only,
        )
    ...
    def from_pretrained(
        ...
        use_usp=False,
        init_concept_adapters: bool = False,
        num_concept_adapters: int = 1,
        init_concept_adapters_moe: Union[bool, List[bool]] = False,
        init_absorbing_token: bool = False,
    ):
        # Redirect model path
        ...
        # Add concept adapters if needed
        if init_concept_adapters:
            pipe.concept_adapter_dict = nn.ModuleDict()

            if num_concept_adapters > 1:
                for i in range(num_concept_adapters):
                    pipe.concept_adapter_dict[f"global_concept_adapter_{i}"] = (
                        ConceptAdapter(
                            in_dim=pipe.dit.dim,
                            out_dim=pipe.dit.dim,
                            add_moe=init_concept_adapters_moe[i],
                        )
                    )
                    pipe.concept_adapter_dict[f"per_block_concept_adapters_{i}"] = (
                        nn.ModuleList(
                            [
                                ConceptAdapter(
                                    in_dim=pipe.dit.dim,
                                ...
                                for _ in range(len(pipe.dit.blocks))
                            ]
                        )
                    )
            else:
                add_moe = (
                    init_concept_adapters_moe
                    if isinstance(init_concept_adapters_moe, bool)
                    else init_concept_adapters_moe[0]
                )
                pipe.concept_adapter_dict["global_concept_adapter"] = ConceptAdapter(
                    in_dim=pipe.dit.dim,
                    out_dim=pipe.dit.dim,
                    add_moe=add_moe,
                )
                pipe.concept_adapter_dict["per_block_concept_adapters"] = nn.ModuleList(
                    [
                        ConceptAdapter(
                            in_dim=pipe.dit.dim,
                            out_dim=pipe.dit.dim,
                ...
                if init_absorbing_token:
                    pipe.concept_adapter_dict["absorbing_token"] = nn.Embedding(
                        1, pipe.dit.dim
                    )

            pipe.concept_adapter_dict.to(
                device=model_config.offload_device or device,
                dtype=model_config.offload_dtype or torch_dtype,
            )
    ...
    def __call__(
        ...
        inputs_posi = {
            "prompt": prompt,
            "multi_concept_prompt": isinstance(prompt, list),
            "tea_cache_l1_thresh": tea_cache_l1_thresh, "tea_cache_model_id": tea_cache_model_id, "num_inference_steps": num_inference_steps,
        }
        inputs_nega = {
            "negative_prompt": negative_prompt,
            "multi_concept_prompt": False,
            "tea_cache_l1_thresh": tea_cache_l1_thresh, "tea_cache_model_id": tea_cache_model_id, "num_inference_steps": num_inference_steps,
        }
        inputs_shared = {
        ...
        models_wo_concept_adapter = {
            name: model
            for name, model in models.items()
            if name != "concept_adapter_dict"
        }
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if cfg_scale != 1.0:
                if cfg_merge:
                    ...
                    noise_pred_nega = self.model_fn(**models_wo_concept_adapter, **inputs_shared, **inputs_nega, timestep=timestep)
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            seperate_cfg=True,
            input_params_posi={"prompt": "prompt", "positive": "positive", "multi_concept_prompt": "multi_concept_prompt"},
            input_params_nega={"prompt": "negative_prompt", "positive": "positive", "multi_concept_prompt": "multi_concept_prompt"},
            onload_model_names=("text_encoder",)
        )

    def process(self, pipe: WanVideoPipeline, prompt, positive, multi_concept_prompt) -> dict:
        pipe.load_models_to_device(self.onload_model_names)
        if multi_concept_prompt:
            prompt_emb, concept_mask = pipe.prompter.encode_prompt(
                prompt, positive=positive, device=pipe.device, multi_concept_prompt=True
            )
            return {"context": prompt_emb, "concept_mask": concept_mask}
        else:
...
def model_fn_wan_video(
    ...
    concept_adapter_dict: Optional[nn.ModuleDict] = None,
    concept_mask: Optional[torch.Tensor] = None,
    concept_adapter_global_only: bool = False,
    **kwargs,
):
    ...
    context_len = context.sum(dim=-1).not_equal(0).sum(dim=-1) - 1
    context_mask = torch.zeros(
        context.shape[0], context.shape[1], dtype=context.dtype, device=context.device
    )
    for i in range(context.shape[0]):
        context_mask[i, :context_len[i]] = 1.0
    context = dit.text_embedding(context)

    if concept_adapter_dict is not None:
        for i in range(context.shape[0]):
            if "absorbing_token" in concept_adapter_dict:
                index_tensor = torch.zeros((1, 1), dtype=torch.long, device=context.device)
                context[i, context_len[i]] = concept_adapter_dict["absorbing_token"](index_tensor)
                context_mask[i, context_len[i]] = 1.0
                context_len[i] += 1

    per_block_concept_adapters_list = []
    if concept_adapter_dict is not None:
        global_concept_adapters = []
        if "global_concept_adapter" in concept_adapter_dict:
            global_concept_adapters.append(concept_adapter_dict["global_concept_adapter"])
        else:
            num_concept_adapters = len(concept_adapter_dict.keys()) // 2
            for i in range(num_concept_adapters):
                global_concept_adapters.append(concept_adapter_dict[f"global_concept_adapter_{i}"])

        if not concept_adapter_global_only:
            if "per_block_concept_adapters" in concept_adapter_dict:
                per_block_concept_adapters_list.append(concept_adapter_dict["per_block_concept_adapters"])
            else:
                num_concept_adapters = len(concept_adapter_dict.keys()) // 2
                for i in range(num_concept_adapters):
                    per_block_concept_adapters_list.append(concept_adapter_dict[f"per_block_concept_adapters_{i}"])

        for i, global_concept_adapter in enumerate(global_concept_adapters):
            delta_context = global_concept_adapter(context) * context_mask.unsqueeze(-1)
            if concept_mask is not None:
                local_mask = concept_mask.eq(i + 1).to(torch.long)
                delta_context = delta_context * local_mask.unsqueeze(-1)
            context = context + delta_context
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            for i, per_block_concept_adapters in enumerate(per_block_concept_adapters_list):
                if block_id < len(per_block_concept_adapters):
                    delta_context = per_block_concept_adapters[block_id](context) * context_mask.unsqueeze(-1)
                    if concept_mask is not None:
                        local_mask = concept_mask.eq(i + 1).to(torch.long)
                        delta_context = delta_context * local_mask.unsqueeze(-1)
                    block_context = block_context + delta_context
            ...
            if use_gradient_checkpointing_offload:
                ...
                x = block(x, block_context, t_mod, freqs)
