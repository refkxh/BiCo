...
class ConceptAdapter(nn.Module):
    ...
    def __init__(
        ...
        self.proj = build_expert()
        self.scaling = nn.Parameter(torch.zeros(1))

        self.use_moe = add_moe
        if add_moe:
            self.proj_motion = build_expert()
            self.gate = nn.Sequential(
                nn.LayerNorm(in_dim),
                nn.Linear(in_dim, 1),
                nn.Softmax(dim=-1)
            )
            self.gate[1].bias.data.fill_(2.0)

    def forward(self, x):
        if self.use_moe:
            gate_score = self.gate(x)
            x1 = self.proj(x)
            x2 = self.proj_motion(x)
            x = gate_score * x1 + (1 - gate_score) * x2
            return x * self.scaling.to(dtype=x.dtype, device=x.device)
        else:
            return self.proj(x) * self.scaling.to(dtype=x.dtype, device=x.device)
